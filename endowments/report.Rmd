...
(endowments %>%
  ...
  highlight(on = "plotly_selected", persistent = TRUE, selectize = TRUE) %T>%
...
(endowments_eoy %>%
    ggplot() +
    geom_area(mapping = aes(x=lubridate::year(Date),
                            y=`Distribution`/1000000,
                            fill = Fund,
                            group = Fund,
                            text = paste(Fund,"\n",
                                         lubridate::year(Date), "Distribution:", format.money(round(`Distribution`)))),
              show.legend = FALSE) +
    scale_fill_hue(c = 50, 
                   l = 70, 
                   h=c(270, 150)) +
    geom_line(data = endowments_net_eoy,
              mapping = aes(x = lubridate::year(Date),
                            y = `Distribution`/1000000,
                            text = paste(lubridate::year(Date), "Corpus Distribution:", format.money(round(`Distribution`)))),
              group = 1,
              inherit.aes = FALSE,
              linetype = 1) +
    geom_line(data = endowments_net_eoy,
